...
<template>
    ...
    <div class="min-h-screen bg-ghost flex flex-col relative overflow-hidden max-w-md mx-auto shadow-2xl">
        
        <div class="bg-surface px-4 py-4 shadow-sm z-10">
            <div class="flex justify-between items-center mb-2">
                <Link :href="route('inspections.my-tasks')" class="text-ink-light hover:text-ink-light">
                    <X class="w-6 h-6" />
                </Link>
                <div class="text-xs font-bold uppercase tracking-widest text-ink-light">
                    {{ asset?.code || 'ASSET' }}
                </div>
                <div class="w-6"></div> 
            </div>
            
            <div class="w-full bg-ghost h-2 rounded-full overflow-hidden">
                <div 
                    class="bg-primary h-full transition-all duration-500 ease-out"
                    :style="{ width: progressPercent + '%' }"
                ></div>
            </div>
            <div class="flex justify-between mt-1 text-xs text-ink-light font-medium">
            ...
            <transition :name="direction === 'forward' ? 'slide-left' : 'slide-right'" mode="out-in">
                ...
                <div :key="'step-' + currentStep" class="w-full">
                    ...
                    <template v-if="!isNotesStep && currentParam">
                        <h2 class="text-2xl font-bold text-ink leading-tight mb-2">
                            {{ currentParam.label }}
                        </h2>
                        
                        <p v-if="currentParam.input_type === 'number'" class="text-sm text-ink-light mb-8">
                            Pastikan jumlah fisik sesuai dengan standar.
                        </p>
                        <p v-else class="text-sm text-ink-light mb-8">
                        ...
                        <div v-if="currentParam.input_type === 'radio'" class="space-y-4">
                            <template v-for="(option, idx) in (typeof currentParam.options === 'string' ? JSON.parse(currentParam.options) : currentParam.options)" :key="idx">
                                <button 
                                    ...
                                    class="w-full p-4 rounded-md border-2 text-left transition-all duration-200 flex items-center justify-between group active:scale-[0.98]"
                                    :class="form.answers_map[currentParam.id] === option 
                                        ? 'border-primary bg-primary/10 shadow-md' 
                                        : 'border-ghost-hover bg-surface hover:border-primary'"
                                >
                                    <span class="text-lg font-medium capitalize"
                                        :class="form.answers_map[currentParam.id] === option ? 'text-primary' : 'text-ink dark:text-ink-dark/90'"
                                    >
                                        {{ option }}
                                    </span>
                        ...
                        <div v-else-if="currentParam.input_type === 'number'" class="flex flex-col items-center">
                            <div class="mb-8 flex flex-col items-center">
                                <span class="text-xs font-bold text-ink-light uppercase tracking-widest mb-1">Standar Minimal</span>
                            ...
                            <div class="flex items-center gap-4 mb-8">
                                <button 
                                    ...
                                    class="w-16 h-16 rounded-full bg-surface border-2 border-ghost-hover text-ink-light hover:border-gray-400 hover:text-ink-light flex items-center justify-center text-4xl font-light shadow-sm active:scale-90 transition"
                                >
                                    -
                                </button>

                                <div class="flex flex-col items-center w-24">
                                    <span class="text-6xl font-black text-ink tabular-nums">
                                        {{ form.answers_map[currentParam.id] }}
                                    </span>
                                    <span class="text-xs font-bold text-ink-light uppercase mt-1">Stok Fisik</span>
                                </div>

                                <button 
                                    @click="adjustNumber(1)"
                                    class="w-16 h-16 rounded-full bg-primary text-white flex items-center justify-center text-4xl font-light shadow-lg shadow-indigo-200 active:scale-90 transition hover:bg-primary-hover"
                                >
                                    +
                                </button>
                            </div>
                            ...
                            <div v-if="parseInt(form.answers_map[currentParam.id]) < currentParam.standard_qty" 
                                 class="flex items-center gap-2 text-warning bg-warning/10 px-4 py-3 rounded-md border border-orange-100 animate-pulse">
                                <AlertTriangle class="w-5 h-5" />
                                <span class="font-bold text-sm">Perlu Restock!</span>
                            </div>
                        </div>
                        <div v-else-if="currentParam.input_type === 'date'" class="w-full mt-4">
                            <div class="text-sm font-medium text-warning bg-warning/10 p-3 rounded-md mb-3 border border-amber-100 flex items-center gap-2">
                            ...
                            <input 
                                ...
                                class="w-full p-4 rounded-md border-2 border-ghost-hover focus:border-primary focus:ring-2 focus:ring-indigo-100 transition-all text-xl text-ink dark:text-ink-dark/90 font-medium bg-surface"
                            >
                        </div>
                    </template>
                    ...
                    <template v-else-if="isNotesStep">
                        <div class="flex flex-col items-center text-center mb-6 mt-4">
                            <div class="w-16 h-16 bg-primary/10 text-primary rounded-full flex items-center justify-center mb-4 border border-primary/20 shadow-sm">
                                <FileText class="w-8 h-8" />
                            </div>
                            <h2 class="text-2xl font-bold text-ink leading-tight mb-2">
                                Catatan Akhir
                            </h2>
                            <p class="text-sm text-ink-light px-4">
                                Jelaskan kelainan yang asset disini....
                            </p>
                        </div>

                        <div class="bg-surface rounded-md border-2 border-ghost-hover overflow-hidden shadow-sm hover:border-primary transition-colors focus-within:border-primary focus-within:ring-2 focus-within:ring-indigo-200">
                            <textarea 
                                v-model="form.global_notes" 
                                rows="5" 
                                class="w-full border-0 p-4 text-ink dark:text-ink-dark/90 placeholder-gray-400 focus:ring-0 resize-none"
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-surface border-t border-ghost-hover p-4 max-w-md mx-auto z-20 flex gap-3">
            
            <button 
                v-if="currentStep > 0"
                @click="prevStep" 
                class="px-4 py-3 rounded-md border border-ghost-hover text-ink-light hover:bg-ghost transition flex items-center justify-center active:scale-[0.98]"
            >
            ...
            <button 
                v-if="!isLastStep"
                @click="nextStep" 
                class="flex-1 bg-gray-900 text-white rounded-md font-bold text-lg shadow-xl shadow-gray-200 hover:bg-gray-800 active:scale-[0.98] transition py-3 flex items-center justify-center gap-2"
            >
                Lanjut
            ...
            <button 
                ...
                class="flex-1 bg-primary text-white rounded-md font-bold text-lg shadow-xl shadow-indigo-200 hover:bg-primary-hover active:scale-[0.98] transition py-3 flex items-center justify-center gap-2"
            >
